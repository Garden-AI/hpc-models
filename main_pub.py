...
#     parser.add_argument('--name', type=str, default=''.join(random.choice(string.ascii_lowercase) for i in range(10)))
    ...
    parser.add_argument('--use-artifacts', action='store_true',
                        help="download model artifacts for loading a model...")
    parser.add_argument('--which_mode', type=str, help="which mode for script?", default="train",
                        choices=["train", "infer", "tuning", "explain", "md", "relax", "ase2mda"])
    parser.add_argument('--zeo_exec', type=str, help="Zeo++ executable",
                        default="/Scr/hyunpark/zeo++-0.3/network")
    parser.add_argument('--zeo_path', type=str,
                        help="Zeo++ file path", default="/Scr/hyunpark/ArgonneGNN/")
    ...
    parser.add_argument('--sync_batch', action='store_true',
                        help="sync batchnorm")  # normalize energy???
    # normalize energy???
    parser.add_argument('--data_norm', action='store_true')
    parser.add_argument('--dataset', type=str, default="qm9edge", choices=[
                        "qm9", "md17", "ani1", "ani1x", "qm9edge", "s66x8", "moleculenet", "cifdata", "gandata", "cifdata_original", "redox"])
    parser.add_argument('--data_dir', type=str,
                        default="/Scr/hyunpark/ArgonneGNN/ligand_data")
    parser.add_argument('--ase_save_dir', type=str,
                        default="/Scr/hyunpark/ArgonneGNN/argonne_gnn_gitlab/ase_run")
    # parser.add_argument('--data_dir_crystal', type=str, default="/Scr/hyunpark/ArgonneGNN/argonne_gnn/CGCNN_test/data/imax")
    parser.add_argument('--data_dir_crystal', type=str,
                        default="/Scr/hyunpark/ArgonneGNN/hMOF/cifs/")
    parser.add_argument('--task', type=str, default="homo")
    # causes CUDAMemory error;; asynchronously reported at some other API call
    ...
    parser.add_argument('--make_data', action="store_true",
                        help="force making data")
    parser.add_argument('--save_to_pickle', type=str,
                        default=None, help="whether to save CIFDataset")
    parser.add_argument('--num_oversample', type=int, default=1000,
                        help="number of oversampling for minority")  # -1 for all
    parser.add_argument('--custom_dataloader', default=None,
                        help="custom dataloader obj")
    ...
    parser.add_argument('--loss_schedule', '-ls', type=str, choices=[
                        "manual", "lrannealing", "softadapt", "relobralo", "gradnorm"], help="how to adjust loss weights.")
    parser.add_argument('--with_force', type=bool, default=False)
    parser.add_argument('--optimizer', type=str, default='adam', choices=[
                        "adam", "lamb", "sgd", "torch_adam", "torch_adamw", "torch_sparse_adam"])
    ...
    # model
    parser.add_argument('--backbone', type=str, default='physnet', choices=["schnet", "physnet", "torchmdnet", "alignn", "dimenet",
                        "dimenetpp", "cgcnn", "cgcnn_original", "calignn", "cphysnet", "cschnet", "ctorchmdnet", "megnet", "mpnn", "graph_transformer"])
    parser.add_argument('--load_ckpt_path', type=str,
                        default="/Scr/hyunpark/ArgonneGNN/argonne_gnn_gitlab/save_pub/")
    # Only for Schnet.Physnet.Alignn WIP!
    parser.add_argument('--explain', type=bool, default=False,
                        help="gradient hook for CAM...")
    parser.add_argument('--dropnan', action="store_true",
                        help="drop nan smiles... useful for ligand model! during inference!")

    # hyperparameter optim
    parser.add_argument('--resume_hp', action="store_true",
                        help="resume hp search if discontinued...")
    parser.add_argument('--hp_savefile', type=str, default="results.csv",
                        help="resume hp search from this file...")
    parser.add_argument('--hyper_inputs', type=str,
                        default="hyper_inputs.yaml", help="tuning inputs..")
    parser.add_argument('--save_hyper_dir', type=str,
                        default=os.getcwd(), help="where to save hyper results.csv")

    # explain options
    parser.add_argument('--n_components', type=int, default=2,
                        help="Dimension reduction", choices=[2, 3])
    parser.add_argument('--clust_num', type=int,
                        default=8, help="cluster numbers")
    parser.add_argument('--clust_algo', type=str, default="kmeans",
                        help="which algo for cluster", choices=["kmeans", "dbscan", "bgm"])
    parser.add_argument('--proj_algo', type=str, default="umap",
                        help="which algo for reduction", choices=["umap", "tsne", "pca"])
    parser.add_argument('--color_scheme', type=str, default="pred",
                        help="which value to color on scatter", choices=["real", "pred", "clust"])
    # parser.add_argument('--render_option', type=str, default="plotly", help="how to save")
    parser.add_argument('--infer_for', type=str, default="ligand",
                        help="infer_for crystal or ligand", choices=["ligand", "crystal"])
    parser.add_argument('--which_explanation', default="projection", help="what to highlight",
                        choices=["projection", "embedding", "interactive", "decomposition"])
    parser.add_argument('--models_to_explain', default=[
                        "physnet", "schnet", "mpnn", "megnet"], help="list of models to explain", nargs="*")

    # md options
    parser.add_argument('--heat_bath', type=float, default=None,
                        help="Temperature in Kelvin... None is NVE; float is NVT")
    parser.add_argument('--timestep', type=float, default=0.1,
                        help="in fs unit of intergration time")
    parser.add_argument('--md_steps', type=int, default=200,
                        help="how many frames (aka md integration steps)")
    parser.add_argument('--md_name', type=str,
                        default="test", help="saved md traj name")
    parser.add_argument('--mda_format', type=str, default="dcd",
                        help="save to a different traj format and return MDA Universe...")
    parser.add_argument('--smiles_list', default=["CC(=O)O", "C1CC1N2C=C(C(=O)C3=CC(=C(C=C32)N4CCNCC4)F)C(=O)O",
                        "Oc1ccc(cc1)C=Cc1cc(O)cc(c1)O"], nargs="*", help="list of SMILES")
...
def call_model(opt: argparse.ArgumentParser, mean: float, std: float, logger: WandbLogger, return_metadata=False):
    ...
    model_kwargs = BACKBONE_KWARGS.get(
        opt.backbone, None)  # TorchMDNet not yet!

    if opt.backbone not in ["calignn", "alignn"]:
        model_kwargs.update({"explain": opt.explain})
    else:
        # only for alignn net (due to pydantic)
        model_kwargs.explain = opt.explain
    ...
    if opt.backbone in ["schnet", "physnet", "dimenet", "dimenetpp", "cschnet", "cphysnet", "cgcnn", "cgcnn_original", "megnet", "mpnn"]:
        ...
        model = torchmdnet.create_model(
            model_kwargs, mean=mean, std=std) if opt.backbone == "torchmdnet" else ctorchmdnet.create_model(model_kwargs)
    ...
    load_state(model, optimizer=None, scheduler_groups=None, path_and_name=path_and_name,
               model_only=True, use_artifacts=False, logger=logger, name=None)
...
def call_loader(opt: argparse.ArgumentParser):
    ...
    if opt.dataset in ["qm9edge"]:
        ...
        # For jake's bias exists; for hMOF, bias is None...
...
def run():
    ...
    if opt.log:
        logger = WandbLogger(
            name=None, entity="argonne_gnn", project='internship')
        ...
        os.environ["WANDB_CACHE_DIR"] = os.path.join(
            os.getcwd(), ".cache/wandb")
        os.environ["WANDB_CONFIG_DIR"] = os.path.join(
            os.getcwd(), ".config/wandb")
    else:
        logger = None
...
def most_commom_substructure(opt: argparse.ArgumentParser, summary: collections.namedtuple, logger: WandbLogger):
    ...
    BRICS vs MMPA
    """
    assert not opt.dataset in [
        "cifdata", "gandata", "cifdata_original"], "MCS cannot be acquired for crystals..."
    clust_num = opt.clust_num
    list_of_idx = list(map(lambda inp: np.where(
        summary.clustered == inp)[0], np.arange(clust_num)))
    smiles_to_mols = np.array(list(map(lambda inp: Chem.MolFromSmiles(
        inp), summary.names)))  # to ndarray for indexing
    mcs_list = list(map(lambda inp: rdFMCS.FindMCS(smiles_to_mols[np.array(
        inp)].tolist()).smartsString, list_of_idx))  # list of smartsstring...
    mcs_to_mols = list(map(lambda inp: Chem.MolFromSmarts(inp), mcs_list))
    mols_to_wimgs = list(map(lambda i, inp: mols_to_wimgs_func(
        i, inp), np.arange(len(mcs_to_mols)), mcs_to_mols))
...
def explain(smiles_list: List[str] = ["CC(=O)O", "C1CC1N2C=C(C(=O)C3=CC(=C(C=C32)N4CCNCC4)F)C(=O)O", "Oc1ccc(cc1)C=Cc1cc(O)cc(c1)O"]):
    ...
    if opt.which_explanation in ["embedding"]:
        ...
        if not opt.dataset in ["cifdata", "gandata", "cifdata_original"]:  # , "Visualization for EMBEDDING method for CRYSTALS are not supported yet..."
            ...
            model, radius_cutoff, max_num_neighbors = call_model(
                opt, mean, std, logger, return_metadata=True)
            if not opt.dropnan:
                print(
                    "Embedding explanation requires non-Nan SMILES, reverting to dropnan...")
                opt.dropnan = True
            else:
                pass
            if smiles_list[0] == "None":
                ...
                # contains nans
                smiles_list, * \
                    _ = preprocess(infer_for_method, df,
                                   test_loader, model, return_vecs=True)
                # Due to sheer number of smiles list...
                smiles_list = smiles_list[:100].tolist()
            else:
                print("Smiles list is provided!")
            logger.log_explainer(model=model, smiles_list=smiles_list, device=device,
            ...
            fig = viz_mof_cif_v2(os.path.join(
                opt.data_dir_crystal, "DB12-ODODIW_clean.cif"))
            # overwrite is ok...
            path_html = "plotly_visualization_output.html"
            fig.write_html(path_html, auto_play=False)
            # For each model column, get multiple index rows of color schemes
        ...
        df = pd.DataFrame()
        df, X = infer_for_method(
            df, test_loader, model, return_vecs=True)  # df and numpy
        mapper = KeplerMapper()
        clusters = KMeans().fit_transform(X)
        X_projected = mapper.fit_transform(
            X, projection=[UMAP(n_components=2)])
        # SPEED-UP: 1. less Cover cubes (=computation);; 2. less Cover overlap (=edges)
        # REDUCED dimension: X_projected accelerates graph generation
        graph = mapper.map(
            X_projected, X, clusterer=KMeans(), cover=Cover(10, 0.5))
        ...
        html = mapper.visualize(graph, color_values=np.stack([df.pred.values.reshape(-1,), df.real.values.reshape(-1,), clusters.argmax(axis=-1).reshape(-1,), clusters.argmin(axis=-1).reshape(-1,)], axis=1), color_function_name=[
                                'pred', 'real', 'cluster_max', 'cluster_min'], path_html=path_html, colorscale=colorscale_from_matplotlib_cmap(plt.cm.jet, nbins=255), custom_tooltips=df.name.values.reshape(-1,), include_searchbar=True)
        logger.log_html(path_html)
        ...
        for model_name in opt.models_to_explain:
            opt.name = model_name
            backbone = re.split('_pub_|_pub', model_name)[0] if "pub" in model_name else model_name.split(
                "_")[0]  # WIP: must change at some point! split no either patterns
            opt.backbone = backbone
            model = call_model(opt, mean, std, logger)
            df = pd.DataFrame()
            # df, X = infer_for_ligand(df, test_loader, model, return_vecs=True)
            # print(df, X)
            names, lat_vecs, preds, reals = preprocess(
                infer_for_method, df, test_loader, model, return_vecs=True)
            for color_scheme in color_scheme_list:
                fig, summary = update_graph(names, lat_vecs, preds, reals, opt.clust_num,
                                            opt.clust_algo, opt.proj_algo, color_scheme, opt.n_components, model_name)
                # overwrite is ok...
                path_html = "plotly_visualization_output.html"
                fig.write_html(path_html, auto_play=False)
                # For each model column, get multiple index rows of color schemes
                html_dict[model_name].append(wandb.Html(open(path_html)))
                if color_scheme == "clust" and not opt.dataset in ["cifdata", "gandata", "cifdata_original"]:
                    mcs_df_list.append(most_commom_substructure(
                        opt, summary, logger))  # append a DF
                else:
                    pass
            ri_means, ri_stds, ri_metrics = roughness_index(
                names, lat_vecs, preds, reals, opt.clust_num, opt.clust_algo, opt.proj_algo, color_scheme, opt.n_components, model_name)
            rogi_dict[model_name].extend(ri_means.tolist())
            rogi_dict[model_name].extend(ri_stds.tolist())
        rogi_dict["Rogi Metric and Statistics"].extend(
            [ri_met + "_mean" for ri_met in ri_metrics])
        rogi_dict["Rogi Metric and Statistics"].extend(
            [ri_met + "_std" for ri_met in ri_metrics])
        # ROWS: prop&clust&ROGImean&ROGIstd
        dataframe = pd.DataFrame.from_dict(html_dict)
        # + ["ROGI_mean", "ROGI_std"]
        dataframe["color_scheme"] = color_scheme_list
        dataframe = dataframe.set_index("color_scheme")
        logger.log_dataframe(dataframe, "dimension reduction")
        # ROWS: prop&clust&ROGImean&ROGIstd
        dataframe = pd.DataFrame.from_dict(rogi_dict)
        ...
        if not opt.dataset in ["cifdata", "gandata", "cifdata_original"]:
            mcs_df = pd.concat(mcs_df_list, axis=0).reset_index().drop(
                columns="index")
        ...
        model, radius_cutoff, max_num_neighbors = call_model(
            opt, mean, std, logger, return_metadata=True)
        df = pd.DataFrame()
        smiles_list, *_ = preprocess(infer_for_method, df,
                                     test_loader, model, return_vecs=True)  # contains nans
        assert len(smiles_list) == len(test_loader.dataset)
        # WIP: match the smiles!
        for i, data_batch in enumerate(test_loader):
            temp_fwd = model(z=data_batch["z"].to(torch.cuda.current_device()), pos=data_batch["pos"].to(
                # forward flush! and get Energy
                torch.cuda.current_device()), batch=data_batch["batch"].to(torch.cuda.current_device()))[0]
            temp_fwd.backward(gradient=torch.ones_like(
                temp_fwd))  # forward flush!
            batch_unique, batch_counts = data_batch["batch"].to(
                torch.cuda.current_device()).unique(return_counts=True)
            ...
            smiles_chunk = smiles_list[batch_unique.detach(
            ).cpu().numpy() + i*len(data_batch["batch"])]
            print(len(smiles_chunk), len(data_batch["batch"]))
            metadata_list = list(map(lambda batch_index, smiles, end_idx: plot_explanations(model, data_batch["batch"].to(torch.cuda.current_device(
            )), batch_index, smiles, end_idx, edge_index, return_metadata=True),  batch_unique, smiles_list[batch_unique.detach().cpu().numpy() + i*len(batch_unique)], batch_counts_cumsum))
...
def infer_for_crystal(opt, df, dataloader, model, return_vecs=False):
    ...
    for one_data_batch in dataloader:
        ...
        e = model(data_batch.x, data_batch.edge_attr, data_batch.edge_index,
        ...
        if return_vecs:
            final_conv_acts_list.append(scatter(
                src=model.final_conv_acts, index=data_batch.batch, dim=0, reduce="mean").detach().cpu().numpy())

        df = pd.concat([df, pd.DataFrame(data=np.concatenate([np.array(data_names).reshape(-1, 1), energies.detach().cpu().numpy(
        ).reshape(-1, 1), y.detach().cpu().numpy().reshape(-1, 1)], axis=1), columns=["name", "pred", "real"])], axis=0, ignore_index=True)
    # print(df)

    select_nans = np.where(df.name.values == "nan")[0]  # only rows
    select_nonans = np.where(df.name.values != "nan")[0]  # only rows
    df = df.drop(index=select_nans.tolist()).reset_index().drop(
        columns="index") if opt.dropnan else df
    if return_vecs:
...
def infer_for_ligand(opt, df, dataloader, model, return_vecs=False):
    ...
    for data_batch in dataloader:
        ...
        for idx, bat in enumerate(batch.unique()):
            try:
                one_smiles = Chem.MolToSmiles(dict2rdkitmol(
                    {"z": z[bat == batch], "pos": pos[bat == batch]}))
        ...
        valid_batch_location = torch.isin(batch, torch.LongTensor(
            valid_batch).to(batch))  # Filtered out batch (num_atoms)
        valid_idx_location = torch.LongTensor(valid_batch).to(
            batch)  # Filtered out batch (num_mols)
        # print(valid_idx_location)
        batch = batch[valid_batch_location]
        _, unique_counts = batch.unique(return_counts=True)
        batch = torch.repeat_interleave(torch.arange(valid_idx_location.size(0)).to(
            batch), unique_counts)  # IMPORTANT!: CORRECT way of getting reduced batches!
        # print(batch.unique().shape)
        # choose valid atoms (hence, molecules) (num_atoms)
        z = z[valid_batch_location]
        # choose valid atoms (hence, molecules) (num_atoms)
        pos = pos[valid_batch_location]
        # print(batch.unique().shape, z.shape, pos.shape)

        y = data_batch["E"].to(torch.cuda.current_device())[
            valid_idx_location]  # (num_mols)
        e_and_f = model(z=z, pos=pos, batch=batch)
        energies = e_and_f[0]
        forces = e_and_f[1]
        if return_vecs:
            final_conv_acts_list.append(scatter(
                src=model.final_conv_acts, index=batch, dim=0, reduce="sum").detach().cpu().numpy())
        # print(final_conv_acts_list)
        # print(energies, data_names.shape, len(energies), len(y))
        df = pd.concat([df, pd.DataFrame(data=np.concatenate([data_names, energies.detach().cpu().numpy().reshape(-1, 1),
                       y.detach().cpu().numpy().reshape(-1, 1)], axis=1), columns=["name", "pred", "real"])], axis=0, ignore_index=True)
...
def infer(smiles_list: List[str] = ["CC(=O)O", "C1CC1N2C=C(C(=O)C3=CC(=C(C=C32)N4CCNCC4)F)C(=O)O", "Oc1ccc(cc1)C=Cc1cc(O)cc(c1)O"]):
    is_distributed = init_distributed()
    local_rank = get_local_rank()
    opt = get_parser()
    ...
    if not smiles_list[0] == "None":
        assert isinstance(smiles_list, (tuple, list)) and isinstance(
            smiles_list[0], str), "SMILES strings are not provides!"
        # torch_geometric list of Data instance
        data_batch = list(map(from_smiles, smiles_list))
        data_batch = Batch.from_data_list(data_batch)  # Batch instance
        if opt.gpu:
            data_batch = data_batch.to(torch.cuda.current_device())
        # forward flush! and get Energy
        e_and_f = model(z=data_batch.z, pos=data_batch.pos,
                        batch=data_batch.batch)
        energies = e_and_f[0]
        forces = e_and_f[1]
        df = pd.DataFrame(data=energies.detach().cpu(
        ).numpy().reshape(-1, 1), columns=["property"])
    ...
    df.to_csv(os.path.join(os.getcwd(), "publication_figures",
              f"{opt.name}_property_prediction.csv"))
    print(f"Property is predicted and saved as {
          opt.name}_property_prediction.csv ...")
...
def md(smiles_list: List[str]):
    ...
    shutil.move(f"./{opt.md_name}.xyz", os.path.join(opt.ase_save_dir,
                f"{opt.md_name}.xyz"))  # move to ase_run directory
    # opt.ase_save_dir is ase_run directory
    ase_run = AseInterface(molecule_path=f"{
                           opt.md_name}.xyz", ml_model=model, working_dir=opt.ase_save_dir)

    if opt.which_mode in ["md"]:
        ase_run.init_md(name=opt.md_name, time_step=opt.timestep,
                        temp_bath=opt.heat_bath)
        ...
        u = ase_run.convert_to_mdanalysis(ase_traj=os.path.join(opt.ase_save_dir, f"{
                                          opt.md_name}.traj"), slices=slice(0, None, 100), save_format=opt.mda_format)
        print(u.trajectory)
...
def dict2rdkitmol(input_dict):
    ...
    import pandas as pd
    df = pd.DataFrame([input_dict["z"].tolist()] +
                      input_dict["pos"].T.tolist()).T
