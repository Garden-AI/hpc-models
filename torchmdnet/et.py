...
__all__ = ["TorchMD_ET", "EquivariantMultiHeadAttention"]
...
class TorchMD_ET(nn.Module):
    ...
    def forward(self,
        ...
        for attn in self.attention_layers:
            dx, dvec, oneattn = attn(x, vec, edge_index, edge_weight, edge_attr, edge_vec)
            x = x + dx
            vec = vec + dvec
        x = self.out_norm(x)

        return x, vec, z, pos, batch, oneattn
...
class EquivariantMultiHeadAttention(MessagePassing):
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        if self.explain:
            row, col = edge_index.unbind()
            q_i = q[row]
            k_j = k[col]
            if dk is None:
                oneattn = (q_i * k_j).sum(dim=-1)
            else:
                oneattn = (q_i * k_j * dk).sum(dim=-1)
        else:
            oneattn = None
