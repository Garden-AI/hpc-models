...
#     print(representation_model, output_model)
...
class TorchMD_Net(nn.Module):
    def __init__(
        ...
        if self.explain:
            ...
            self.representation_model.embedding.register_backward_hook(hook)
    ...
    def forward(self,
        ...
        x, v, z, pos, batch, oneattn = self.representation_model(z, pos, batch)
        
        if self.explain:
            self.final_conv_acts = x
            self.attention_weight = oneattn
            def hook(grad):
