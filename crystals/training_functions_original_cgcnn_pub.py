...
def single_train(args, model, loader, loss_func, epoch_idx, optimizer, scheduler, grad_scaler, local_rank, logger: WandbLogger, tmetrics, ameters):
    ...
    for step, pack in tqdm(enumerate(loader), total=len(loader), unit='batch',
                         desc=f'Epoch {epoch_idx}', disable=(args.silent or local_rank != 0)):
        if args.gpu and args.use_tensors:
            assert args.backbone in ["cphysnet","cschnet","cgcnn","cgcnn_original","ctorchmdnet", "calignn"], "Wrong data format for a given backbone model!"
            atom_fea, nbr_fea, nbr_fea_idx, targetE, batch = pack.x, pack.nbr_fea, pack.nbr_fea_idx, pack.target, pack.batch
            pack =  atom_fea, nbr_fea, nbr_fea_idx, targetE, batch
            atom_fea, nbr_fea, nbr_fea_idx, targetE, batch = to_cuda(pack)
            targetE_scaled = (targetE - MEAN) / STD
        else:
            print("Significant error in dataloader!")
            break
		
        with torch.cuda.amp.autocast(enabled=args.amp):
            preds = model(atom_fea, nbr_fea, nbr_fea_idx, batch) 
            loss_mse = loss_func(args, preds, targetE_scaled) #get_loss_func
            loss_metrics = tmetrics(preds.view(-1,).detach().cpu() * STD + MEAN, targetE.view(-1,).detach().cpu()) #LOG energy only!
            ameters.update(loss_metrics, len(preds))
        ...
        if args.log:
            ...
            logger.log_metrics({'rank0_specific_train_loss_mae_ameter': ameters.avg})
...
def single_val(args, model, loader, loss_func, optimizer, scheduler, logger: WandbLogger, tmetrics, ameters):
    model = model.eval()
    _loss = 0
    _loss_metrics = 0.

    with torch.inference_mode():  
        for i, pack in tqdm(enumerate(loader), total=len(loader), unit='batch', desc=f'Evaluation',
            ...
            if args.log:
                ...
                logger.log_metrics({'rank0_specific_val_loss_mae_ameter': ameters.avg})
...
def single_test(args, model, loader, loss_func, optimizer, scheduler, logger: WandbLogger, tmetrics, ameters):
    model = model.eval()
    _loss = 0
    _loss_metrics = 0.

    with torch.inference_mode():  
        for i, pack in tqdm(enumerate(loader), total=len(loader), unit='batch', desc=f'Testing',
		     leave=False, disable=(args.silent or get_local_rank() != 0)):
            ...
            if args.log:
                ...
                logger.log_metrics({'rank0_specific_test_loss_mae_ameter': ameters.avg})
...
#         model = ShardedDDP(model, optimizer) #OOS optimizer and Single-gpu model to ShardedDDP
    ...
    for epoch_idx in range(epoch_start, args.epoches):
        ...
        loss, loss_metrics = train_epoch(args, model, train_dataloader, get_loss_func, epoch_idx, optimizer, scheduler, grad_scaler, local_rank,
                           logger, tmetrics, ameters) #change to single_train with DDP ;; model is AUTO-updated...
        ...
        ###EVALUATION
        evaluate = single_val
        val_loss, loss_metrics = evaluate(args, model, val_dataloader, get_loss_func, optimizer, scheduler, logger, tmetrics, ameters) #change to single_val with DDP
        if dist.is_initialized():
...
#zero rank only
        ...
        ###TESTING
        test_loss, loss_metrics = single_test(args, model, test_dataloader, get_loss_func, optimizer, scheduler, logger, tmetrics, ameters) #change to single_val with DDP
        if dist.is_initialized():
