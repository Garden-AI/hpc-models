...
class ConvLayer(torch.nn.Module):
    ...
    def __init__(self, atom_fea_len, nbr_fea_len):
        ...
        self.bn1 = nn.BatchNorm1d(2*self.atom_fea_len) #; PairNorm; GraphNorm; DiffGroupNorm
    ...
    def forward(self, atom_in_fea, nbr_fea, nbr_fea_idx, batch):
        ...
        total_nbr_fea = torch.cat((atom_self_fea, atom_nbr_fea, nbr_fea), dim=-1) #(edges, DIM)
        total_gated_fea = self.fc_full(total_nbr_fea) #(edges, DIM)
        total_gated_fea = self.bn1(total_gated_fea) #(edges, self.atom_fea_len*2)
        nbr_filter, nbr_core = total_gated_fea.chunk(2, dim=-1)
        # print(nbr_filter.shape, row.shape)
        # nbr_filter = self.act3(nbr_filter.sum(dim=-1), row)[:,None] * nbr_filter ####NEW: attention
        nbr_filter = self.sigmoid(nbr_filter)
        nbr_core = self.act1(nbr_core)
        # nbr_core = self.act3(nbr_core.sum(dim=-1), row)[:,None] * nbr_core #####NEW: attention
        nbr_summed = scatter(src=nbr_filter*nbr_core, index=col, dim=0) #(nodes, dim)
...
class CrystalGraphConvNet(nn.Module):
    ...
    def __init__(self, orig_atom_fea_len=92, nbr_fea_len=41,
        ...
        self.embedding = nn.Linear(orig_atom_fea_len, atom_fea_len) if not learnable else torch.nn.Embedding(120, atom_fea_len)
    ...
    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, dists, crystal_atom_idx, batch=None):
        """
        Forward pass
        ...
        """
        atom_fea = self.embedding(atom_fea) if not self.learnable else self.embedding(crystal_atom_idx) #from dictionary of dataloader!
        # atom_fea = self.embedding(crystal_atom_idx)
        for conv_func in self.convs:
            atom_fea = conv_func(atom_fea, nbr_fea, nbr_fea_idx, batch) #nodes, dim
...
#         return torch.cat(summed_fea, dim=0)
        return scatter(src=atom_fea, index=crystal_atom_idx, dim=0, reduce="mean") #(nmolecules, dim)
