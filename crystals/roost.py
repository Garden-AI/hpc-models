...
#For element training see: (Deep Reinforcement Learning for Inverse Inorganic Materials Design)
#Different from CGCNN
...
class Roost(nn.Module):
    ...
    def forward(self, elem_weights, elem_fea, edge_index, cry_elem_idx):
        """
        Forward pass through the material_nn and output_nn
        """
        crys_fea = self.material_nn(
            elem_weights, elem_fea, edge_index, cry_elem_idx
        )
...
class DescriptorNetwork(nn.Module):
    ...
    def forward(self, elem_weights, elem_fea, edge_index, cry_elem_idx):
        ...
        for graph_func in self.graphs:
            elem_fea = graph_func(elem_weights, elem_fea, edge_index)
...
class MessageLayer(nn.Module):
    ...
    def forward(self, elem_weights, elem_in_fea, edge_index):
        """
        Forward pass
        ...
        """
        self_fea_idx, nbr_fea_idx = edge_index.unbind(dim=0)
