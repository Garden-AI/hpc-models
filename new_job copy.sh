...
ENV_PATH="/projects/bdao/$USER/.conda/envs/phy_env2"

# Check if the environment exists
if [ ! -d "$ENV_PATH" ]; then
    echo "Environment does not exist. Creating environment from environment.yml..."
    conda env create --prefix $ENV_PATH -f env.yml
else
...
conda remove pytorch torch_cluster torch_geometric torch_scatter torch_sparse
conda install pytorch torchvision torchaudio cudatoolkit=11.8 -c pytorch
pip install torch_cluster torch_geometric torch_scatter torch_sparse
